    ...
    while all_ann_num[catId] < shot_num:
        ...
        for i in few_shot_cat_ids:
            if few_shot_ann_num[i] + tmp_few_shot_num[i] > shot_num:
                append_flag = False
        if instances_num_in_img>2 and loop_num<10000:
            append_flag = False
        else:
            append_flag = True
        if append_flag:
...
save_path = f'/home/nieh/morjio/projects/detection/any_shot_detection/mmfewshot/data/any_shot_ann_65_8_7_split/rahman_asd_full_box_{shot_num}shot_trainval_{seed}_v2.json'
