...
import losses
class TrainGAN():
    def __init__(self, opt, attributes, zero_shot_Att, zero_shot_Labels, seen_feats_mean, gen_type='FG'):
        ...
        self.Wu_Labels = np.array([i for i, l in enumerate(zero_shot_Labels)])
        print(f"Wu_Labels {self.Wu_Labels}")
        self.Wu = zero_shot_Att

        self.zero_shot_classifier = ClsUnseen(zero_shot_Att) # 构建unseen的分类器，先预测语义，然后再匹配unseen类别的语义表示得到最终的概率结果，分类的类别数为7，不包含背景类
        self.zero_shot_classifier.cuda()
        # MSCOCO/unseen_Classifier.pth
        self.zero_shot_classifier = loadUnseenWeights(opt.pretrain_classifier_zero_shot, self.zero_shot_classifier)
        ...
        for p in self.zero_shot_classifier.parameters():
            p.requires_grad = False
        ...
        self.netMap = model.Embedding_Net(self.opt)
        ...
        self.contras_criterion = losses.SupConLoss_clear(self.opt.ins_temp)
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            for iter_d in range(self.opt.critic_iter):
                ...
                embed_real, outz_real = self.netMap(input_res)
                ...
                # contrastive loss
                real_ins_contras_loss = self.contras_criterion(outz_real, input_label)
                real_ins_contras_loss.backward()
            ...
            noisev = Variable(noise)
            fake = self.netG(noisev, input_attv)
            embed_fake, outz_fake = self.netMap(fake)
            ...
            fake_ins_contras_loss = self.contras_criterion(all_outz, torch.cat((input_label, input_label), dim=0))
            # ---------------------
            ...
            fake_pred = self.zero_shot_classifier(feats=fake_unseen_f.cuda(), classifier_only=True)

            unseenc_errG = self.opt.cls_weight_zero_shot * self.cls_criterion(fake_pred, Variable(fake_unseen_l.cuda()))
            ...
            errG = -G_cost + c_errG + loss_lz + unseenc_errG + self.opt.ins_weight*fake_ins_contras_loss
            errG.backward()
            self.optimizerG.step()

            print(f"{self.gen_type} [{self.epoch+1:02}/{self.opt.nepoch:02}] [{i:06}/{int(self.ntrain)}] Loss: {errG.item() :0.4f} D loss: {D_cost.data.item():.4f} G loss: {G_cost.data.item():.4f}, W dist: {Wasserstein_D.data.item():.4f} seen loss: {c_errG.data.item():.4f}  unseen loss: {unseenc_errG.data.item():.4f} loss div: {loss_lz.item():0.4f}")
