...
from splits_48_17 import get_unseen_class_ids ,get_seen_class_ids, get_asd_zero_shot_class_ids, get_asd_few_zero_shot_class_ids
...
def load_seen_att_with_bg(opt):
    attribute, labels = load_all_att(opt)
    classes_ids = np.concatenate((get_seen_class_ids(opt.dataset, split=opt.classes_split), [65]))
    return attribute[classes_ids], labels[classes_ids]
