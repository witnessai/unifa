...
class TrainCls():
    ...
    def init_model(self):
        ...
        self.classifier_backup = ClsModelTrain(num_classes=18).cuda()
        # ipdb.set_trace()
    ...
    def trainEpochs(self):
        for epoch in range(self.opt.nepoch_cls):
            ...
            if self.opt.testcls_classifier != self.opt.traincls_classifier:
                tmp_classes_ids = np.concatenate((get_asd_few_zero_shot_class_ids(self.opt.dataset, split=self.opt.classes_split), [65]))
