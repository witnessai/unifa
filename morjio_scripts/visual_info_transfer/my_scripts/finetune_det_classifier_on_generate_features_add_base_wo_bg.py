...
def generate_features(text_embedding, netG):
    ...
    input_attv = text_embedding[COCO_ALL_CLASSES]
    noise = get_z_random(class_num=len(COCO_ALL_CLASSES))
    noise = noise.cuda()
    input_attv = input_attv.cuda()
    gen_labels = torch.Tensor(COCO_ALL_CLASSES).cuda().unsqueeze(1)
...
gt_feats = gt_feats[gt_labels!=80]
gt_labels = gt_labels[gt_labels!=80]
...
def validation(model, test_dataloader):
    ...
    confusion_matrix_base_few_excluede_bg = np.zeros((2, 2))
    ...
    confusion_matrix_base_few_excluede_bg[0, 0] = np.sum(confusion_matrix[COCO_BASE_CLASSES, COCO_BASE_CLASSES])
    confusion_matrix_base_few_excluede_bg[0, 1] = np.sum(confusion_matrix[np.ix_(COCO_BASE_CLASSES, COCO_NOVEL_CLASSES)])
    confusion_matrix_base_few_excluede_bg[1, 0] = np.sum(confusion_matrix[np.ix_(COCO_NOVEL_CLASSES, COCO_BASE_CLASSES)])
    confusion_matrix_base_few_excluede_bg[1, 1] = np.sum(confusion_matrix[COCO_NOVEL_CLASSES, COCO_NOVEL_CLASSES])
    # ipdb.set_trace()
    ...
    print(confusion_matrix_base_few_excluede_bg)

features = torch.from_numpy(np.load('/home/nieh/morjio/projects/detection/any_shot_detection/mmfewshot/data/coco/any_shot_detection_extracted_feats_and_embedding_classifier/visual_info_transfer/65_15_fsd_split/testsetall/test_0.6_0.3_feats.npy')).cuda()
labels = torch.from_numpy(np.load('/home/nieh/morjio/projects/detection/any_shot_detection/mmfewshot/data/coco/any_shot_detection_extracted_feats_and_embedding_classifier/visual_info_transfer/65_15_fsd_split/testsetall/test_0.6_0.3_labels.npy')).cuda()
# ipdb.set_trace()
...
for epoch in tqdm(range(epoch_num)):
    ...
    optimizer.step()
    validation(classifier_from_det, test_dataloader)
    torch.save({'state_dict': classifier_from_det.state_dict(), 'epoch': epoch}, f"/home/nieh/morjio/projects/detection/any_shot_detection/mmfewshot/work_dirs/fsd_65_15_r101_fpn_coco_10shot-fine-tuning/classifier_best_finetuning_on_gen_feats_add_base_wo_bg_epoch{epoch}.pth")
ipdb.set_trace()


## shell command
# CUDA_VISIBLE_DEVICES=0 python finetune_det_classifier_on_generate_features_add_base_wo_bg.py
