...
from cls_models_48_17 import ClsModel, ClsUnseen
import ipdb 
import numpy as np
class TrainGAN():
    def __init__(self, opt, attributes, class_Att, class_Labels, seen_feats_mean, gen_type='FG'):
        ...
        self.Wu_Labels = np.array([i for i, l in enumerate(class_Labels)])
        print(f"Wu_Labels {self.Wu_Labels}")
        self.Wu = class_Att

        self.novel_classifier = ClsUnseen(class_Att) # 构建unseen的分类器，先预测语义，然后再匹配unseen类别的语义表示得到最终的概率结果，分类的类别数为7，不包含背景类
        self.novel_classifier.cuda()
        # MSCOCO/unseen_Classifier.pth
        self.novel_classifier = loadUnseenWeights(opt.pretrain_classifier_unseen, self.novel_classifier)
        ...
        for p in self.novel_classifier.parameters():
            p.requires_grad = False
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            fake_pred = self.novel_classifier(feats=fake_unseen_f.cuda(), classifier_only=True)
