...
from splits import get_unseen_class_labels, get_asd_zero_shot_class_labels, get_asd_few_zero_shot_class_labels, get_asd_base_few_zero_shot_class_labels
...
def val(dataloader, classifier, criterion, opt, epoch, verbose="Val"):
    ...
    for ite, (in_feat, in_label)  in enumerate(dataloader):
        ...
        if opt.supconloss:
            preds, emb = classifier(feats=in_feat, classifier_only=True)
            preds_all.append(preds.data.cpu().numpy())
            ious = torch.ones(in_label.shape[0]).cuda()
            loss1 = criterion[0](preds, in_label)
            probs = torch.softmax(preds, 1)
            loss2 = criterion[1](probs, in_label, ious)
            loss = loss1 + loss2
        else:
...
def compute_per_class_acc(test_label, predicted_label, opt, verbose="Val"):
    ...
    if opt.testcls_classifier == 'zero_shot_set1':
        classes = np.concatenate((get_asd_zero_shot_class_labels(opt.dataset, split=opt.classes_split, fs_set=1), ['background']))
    elif opt.testcls_classifier == 'zero_shot_set2':
        classes = np.concatenate((get_asd_zero_shot_class_labels(opt.dataset, split=opt.classes_split, fs_set=2), ['background']))
    elif opt.testcls_classifier == 'zero_shot_set3':
        classes = np.concatenate((get_asd_zero_shot_class_labels(opt.dataset, split=opt.classes_split, fs_set=3), ['background']))
    elif opt.testcls_classifier == 'few_zero_shot':
        classes = np.concatenate((get_asd_few_zero_shot_class_labels(opt.dataset, split=opt.classes_split), ['background']))
    elif opt.testcls_classifier == 'base_few_zero_shot':
        classes = np.concatenate((get_asd_base_few_zero_shot_class_labels(opt.dataset, split=opt.classes_split), ['background']))
...
def loadFasterRcnnCLSHead(filepath, model):
    ...
    if 'roi_head.bbox_head.fc_cls.weight' in state_dict.keys(): 
        own_dict['fc1.weight'].copy_(state_dict['roi_head.bbox_head.fc_cls.weight'].cuda())
    if 'roi_head.bbox_head.fc_cls.bias' in state_dict.keys(): 
        own_dict['fc1.bias'].copy_(state_dict['roi_head.bbox_head.fc_cls.bias'].cuda())
        assert (model.state_dict()['fc1.bias'] == state_dict['roi_head.bbox_head.fc_cls.bias'].cuda()).all(), 'Something wrong with loading pretrained fasterrcnn cls head!!'
