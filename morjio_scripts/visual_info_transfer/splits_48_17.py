...
COCO_ALL_CLASSES_65 = np.array(['person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat', 'bench', 'bird', 'cat', 'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack', 'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'kite', 'skateboard', 'surfboard', 'bottle', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple', 'sandwich', 'orange', 'broccoli', 'carrot', 'pizza', 'donut', 'cake', 'chair', 'couch', 'bed', 'toilet', 'tv', 'laptop', 'mouse', 'remote', 'keyboard', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator', 'book', 'clock', 'vase', 'scissors', 'toothbrush'])
...
COCO_UNSEEN_CLASSES_48_17 = np.array([
    ...
    ## 从小到大顺序
    'airplane', 'bus', 'cat', 'dog', 'cow', 'elephant', 'umbrella', 'tie', 'snowboard', 'skateboard', 'cup', 'knife', 'cake', 'couch', 'keyboard', 'sink', 'scissors'
])
...
def get_asd_base_few_zero_shot_class_labels(dataset, split='65_8_7'):
    if dataset == 'coco':
        return COCO_ALL_CLASSES if split=='65_8_7' else COCO_ALL_CLASSES_65

def get_asd_base_few_zero_shot_class_ids(dataset, split='65_8_7'):
    if dataset == 'coco':
        BASE_FEW_ZERO_SHOT_CLASSES = COCO_ALL_CLASSES if split=='65_8_7' else COCO_ALL_CLASSES_65
        ids = np.where(np.isin(COCO_ALL_CLASSES_65, BASE_FEW_ZERO_SHOT_CLASSES))[0]
        return ids
