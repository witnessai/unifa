...
from splits import get_asd_zero_shot_class_ids, get_asd_zero_shot_class_labels, get_unseen_class_ids, get_unseen_class_labels, get_asd_base_few_zero_shot_class_labels, get_asd_base_few_zero_shot_class_ids, get_asd_few_zero_shot_class_ids, get_asd_few_zero_shot_class_labels
...
class TrainCls(): 
    def __init__(self, opt):
        ...
        if opt.traincls_classifier == 'zero_shot':
            self.classes = get_asd_zero_shot_class_labels(opt.dataset, split=opt.classes_split)
        elif opt.traincls_classifier == 'few_zero_shot':
            self.classes = get_asd_few_zero_shot_class_labels(opt.dataset, split=opt.classes_split)
        elif opt.traincls_classifier == 'base_few_zero_shot':
            self.classes = get_asd_base_few_zero_shot_class_labels(opt.dataset, split=opt.classes_split)
...
for epoch in range(start_epoch, n_epoch):
    ...
    for iter, l in enumerate(unique_label): # 确保每类的训练样本数量相等
