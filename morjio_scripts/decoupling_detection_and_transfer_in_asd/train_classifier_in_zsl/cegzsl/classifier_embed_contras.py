...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, map_net, embed_size, testsetloader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=64, generalized=True, resnet=None):
        self.train_X =  _train_X 
        self.train_Y = _train_Y 
        self.testsetloader = testsetloader
        self.seenclasses = list(range(65))
        self.unseenclasses = list(range(65, 80))
        self.MapNet=map_net
        ...
        self.optimizerMap = optim.Adam(self.MapNet.parameters(), lr=_lr, betas=(_beta1, 0.999))
    ...
    def fit_gzsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            for i in range(0, self.ntrain, self.batch_size):  
                ...
                # self.input.copy_(batch_input)
                # self.label.copy_(batch_label)
                embed, _ = self.MapNet(batch_input)
                output = self.model(embed)
                loss = self.criterion(output, batch_label)
                loss.backward()
                self.optimizer.step()
            acc_seen = self.val_gzsl( self.seenclasses)
            acc_unseen = self.val_gzsl( self.unseenclasses)
    ...
    def val_gzsl(self, targetclasses): 
        input_label_list = []
        predicted_label_list = []
        for i, data_input in enumerate(self.testsetloader):
            input_img, input_label, imgnames = data_input
            input_img = input_img.cuda()
            input_label = input_label.cuda()
            batch_size = len(input_label)
            input_res = self.resnet(input_img)
            input_res = input_res.reshape(batch_size, -1)
            embed, _ = self.MapNet(input_res)
            output = self.model(embed)
            _, predicted_label = torch.max(output, 1)
            input_label_list.append(input_label)
            predicted_label_list.append(predicted_label)
        input_label = torch.cat(input_label_list)
        predicted_label = torch.cat(predicted_label_list)
        acc = self.compute_per_class_acc(input_label, predicted_label, targetclasses)
        return acc
    ...
    def compute_per_class_acc(self, test_label, predicted_label, target_classes):
        acc_per_class = torch.FloatTensor(len(target_classes)).fill_(0)
        for i in range(len(target_classes)):
            idx = (test_label == target_classes[i])
            if torch.sum(idx) != 0:
