...
class DATA_LOADER(object):
    ...
    def read_matimagenet(self, opt):
        ...
        if opt.validation:
            ...
            self.novelclasses = torch.from_numpy(np.array(matcontent['novel_classes1']).astype(int).squeeze()).long() - 1
            test_idx_baseclasses = torch.from_numpy(np.array(matcontent['idx_baseclasses1']).astype(int).squeeze()).long() - 1
            test_idx_novelclasses = torch.from_numpy(np.array(matcontent['idx_novelclasses1']).astype(int).squeeze()).long() - 1
        else:
            ...
            self.novelclasses = torch.from_numpy(np.array(matcontent['novel_classes2']).astype(int).squeeze()).long() - 1
            test_idx_baseclasses = torch.from_numpy(np.array(matcontent['test_base2_loc']).astype(int).squeeze()).long() - 1
            test_idx_novelclasses = torch.from_numpy(np.array(matcontent['test_novel2_loc']).astype(int).squeeze()).long() - 1
