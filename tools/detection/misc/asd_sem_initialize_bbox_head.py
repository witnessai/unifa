...
# COCO config
COCO_NOVEL_CLASSES = [
    5, 7, 14, 17, 23, 33, 34, 36, 48, 54, 58, 70, 74, 80, 89
]
COCO_BASE_CLASSES = [
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['roi_head.bbox_head.kernel_semantic'],
...
def random_init_checkpoint(param_name, is_weight, tar_size, checkpoint, args):
    ...
    if 'kernel_semantic' in param_name:
        prev_cls -= 1
    if is_weight:
        # feat_size = pretrained_weight.size(1)
        new_weight = torch.rand((tar_size, tar_size))
    ...
    if args.coco or args.lvis:
        ...
        for i, c in enumerate(BASE_CLASSES):
            ...
            if is_weight:
                length = pretrained_weight[idx].shape[0] - 1 
                length_bg = length
                if 'kernel_semantic' in param_name:
                    new_weight[IDMAP[c]][:length] = pretrained_weight[idx][:length]
                    new_weight[IDMAP[c]][-1] = pretrained_weight[idx][-1]
            else:
                if 'kernel_semantic' in param_name:
    ...
    if 'kernel_semantic' in param_name:
        if is_weight:
            new_weight[-1][:length_bg] = pretrained_weight[-1][:length_bg]  # bg class
            new_weight[-1][-1] = pretrained_weight[-1][-1]
        else:
