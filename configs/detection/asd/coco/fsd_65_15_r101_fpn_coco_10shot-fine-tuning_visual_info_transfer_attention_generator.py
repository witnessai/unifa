...
model = dict(
    roi_head=dict(
        type='StandardRoIHead_visual_info_transfer_attention_generator',
        bbox_head=dict(
            type='Shared2FCBBoxHead_visual_info_transfer_attention_generator',
            loss_cls=dict(
                type='CrossEntropyLoss_in_visual_info_transfer_attention_generator', use_sigmoid=False, loss_weight=1.0),
            )
    )
)
...
#              'base_model_random_init_bbox_head.pth')
load_from = ('work_dirs/fsd_65_15_r101_fpn_coco_10shot-fine-tuning/'
             'iter_80000.pth')
