...
COCO_FEW_SHOT_1_CLASSES = [7, 14, 23, 36, 48, 58, 80, 89]
COCO_FEW_SHOT_2_CLASSES = [7, 14, 17, 23, 36, 48, 70, 80]
COCO_FEW_SHOT_3_CLASSES = [7, 14, 17, 23, 36, 48, 74, 80]

## different zero-shot sets
COCO_ZERO_SHOT_1_CLASSES = [5, 17, 33, 34, 54, 70, 74]
COCO_ZERO_SHOT_2_CLASSES = [5, 33, 34, 54, 58, 74, 89]
COCO_ZERO_SHOT_3_CLASSES = [5, 33, 34, 54, 58, 70, 89]


COCO_BASE_CLASSES = [
    1, 2, 3, 4, 6, 8, 9, 10, 11, 13, 15, 16, 18, 19, 20, 21, 22, 24, 25, 27, 28, 31, 32, 35, 37, 38, 39, 40, 41, 42, 43, 44, 46, 47, 49, 50, 51, 52, 53, 55, 56, 57, 59, 60, 61, 62, 63, 64, 65, 67, 72, 73, 75, 76, 77, 78, 79, 81, 82, 84, 85, 86, 87, 88, 90
]
COCO_NOVEL_CLASSES = COCO_FEW_SHOT_1_CLASSES
COCO_BASE_CLASSES = sorted(COCO_BASE_CLASSES + COCO_NOVEL_CLASSES)
COCO_NOVEL_CLASSES = COCO_ZERO_SHOT_1_CLASSES
...
def parse_args():
    ...
    parser.add_argument('--fs_set', type=int, default=1, help='choose few-shot set')
...
def random_init_checkpoint(param_name, is_weight, tar_size, checkpoint, args):
    ...
    """
    if args.fs_set == 2:
        COCO_NOVEL_CLASSES = COCO_ZERO_SHOT_2_CLASSES
        COCO_ALL_CLASSES = sorted(COCO_BASE_CLASSES + COCO_NOVEL_CLASSES)
        COCO_IDMAP = {v: i for i, v in enumerate(COCO_ALL_CLASSES)}
    if args.fs_set == 3:
        COCO_NOVEL_CLASSES = COCO_ZERO_SHOT_3_CLASSES
        COCO_ALL_CLASSES = sorted(COCO_BASE_CLASSES + COCO_NOVEL_CLASSES)
        COCO_IDMAP = {v: i for i, v in enumerate(COCO_ALL_CLASSES)}
...
def main():
    ...
    save_name = args.tar_name + f'_{args.method}_bbox_head_for_fs_set{args.fs_set}.pth'
