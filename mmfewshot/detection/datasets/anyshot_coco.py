...
class AnyShotCocoDataset(BaseFewShotDataset, CocoDataset):
    ...
    Args:
        classes (str | Sequence[str] | None): Classes for model training and
            ...
            it will load pre-defined classes in :obj:`AnyShotCocoDataset`.
    ...
    def __init__(self,
        ...
        assert classes is not None, f'{self.dataset_name}: classes in ' \
                                    f'`AnyShotCocoDataset` can not be None.'
    ...
    def get_classes(self, classes: Union[str, Sequence[str]]) -> List[str]:
        ...
        Args:
            classes (str | Sequence[str]): Classes for model training and
                provide fixed label for each class. When classes is string,
                it will load pre-defined classes in `AnyShotCocoDataset`.
...
@DATASETS.register_module()
class AnyShotCocoCopyDataset(AnyShotCocoDataset):
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=AnyShotCocoDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=AnyShotCocoDataset.data_infos)]
...
@DATASETS.register_module()
class AnyShotCocoDefaultDataset(AnyShotCocoDataset):
    """AnyShot COCO Dataset with some pre-defined annotation paths.

    :obj:`AnyShotCocoDefaultDataset` provides pre-defined annotation files
    ...
    different from :obj:`AnyShotCocoDataset`. The `ann_cfg' should contain
    ...
    coco_benchmark = {
        f'{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'data/any_shot_ann/coco/benchmark_{shot}shot/'
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(AnyShotCocoDataset, self).ann_cfg_parser(new_ann_cfg)
