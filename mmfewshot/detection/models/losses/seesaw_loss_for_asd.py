...
def seesaw_ce_loss_for_asd(cls_score,
                   labels,
                   label_weights,
                   cum_samples,
                   num_classes,
    ...
    # seesaw_weights.shape is [pos_num, num_classes]
    seesaw_weights = cls_score.new_ones(onehot_labels.size())
    
    # mitigation factor
    if p > 0:
        # sample_ratio_matrix.shape is [num_classes, num_classes]
        sample_ratio_matrix = cum_samples[None, :].clamp(
            min=1) / cum_samples[:, None].clamp(min=1)
        index = (sample_ratio_matrix < 1.0).float()
        # sample_weights.shape is [num_classes, num_classes]
...
class SeesawLoss_for_asd(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0,
                 return_dict=True):
        super(SeesawLoss_for_asd, self).__init__()
        ...
        # 0 for pos, 1 for neg
        self.cls_criterion = seesaw_ce_loss_for_asd
    ...
    def forward(self,
        ...
        # cls_score.shape is [1024, 75]
        # cls_score_classes.shape is [1024, 73], cls_score_objectness.shape is [1024, 2]
        cls_score_classes, cls_score_objectness = self._split_cls_score(
            cls_score)
        # calculate loss_cls_classes (only need pos samples)
        # self.loss_weight=1, self.p=0.8, self.q=2.0, self.eps=0.01, avg_factor=1024
        if pos_inds.sum() > 0:
