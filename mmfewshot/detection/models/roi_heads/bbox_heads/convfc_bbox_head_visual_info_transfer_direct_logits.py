...
class ConvFCBBoxHead_visual_info_transfer_direct_logits(BBoxHead2):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCBBoxHead_visual_info_transfer_direct_logits, self).__init__(
    ...
    def forward(self, x):
        ...
        fs_score = self.gen_fs_classifier(x_cls)
        ...
        ## (1) logits相加再平均 含背景
        # for i, fs_idx in enumerate(COCO_FS_CLASSES_AND_BG):
        #     cls_score[:, fs_idx] = (cls_score[:, fs_idx] + fs_score[:, i])/2
        
        ## (2) logits相加再平均 不含背景
        # for i, fs_idx in enumerate(COCO_FS_CLASSES):
        #     cls_score[:, fs_idx] = (cls_score[:, fs_idx] + fs_score[:, i])/2

        ## (3) 将fs_score的占比迁移到cls_score上
        ## 发现这个想法不太好实现，fs_score的logits大多都是负的，负的占比大的，反而softmax的概率是更小的
        # cls_score[:, COCO_FS_CLASSES_AND_BG] / torch.sum(cls_score[:, COCO_FS_CLASSES_AND_BG], dim=1).unsqueeze(1).repeat(1, len(COCO_FS_CLASSES_AND_BG))
        # fs_score[:, :] / torch.sum(fs_score[:, :], dim=1).unsqueeze(1).repeat(1, len(COCO_FS_CLASSES_AND_BG))
        ipdb.set_trace()
...
@HEADS.register_module()
class Shared2FCBBoxHead_visual_info_transfer_direct_logits(ConvFCBBoxHead_visual_info_transfer_direct_logits):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(Shared2FCBBoxHead_visual_info_transfer_direct_logits, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
...
@HEADS.register_module()
class Shared4Conv1FCBBoxHead_visual_info_transfer_direct_logits(ConvFCBBoxHead_visual_info_transfer_direct_logits):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(Shared4Conv1FCBBoxHead_visual_info_transfer_direct_logits, self).__init__(
            num_shared_convs=4,
            num_shared_fcs=1,
            num_cls_convs=0,
            num_cls_fcs=0,
