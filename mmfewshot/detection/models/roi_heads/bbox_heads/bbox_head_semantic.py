...
class BBoxSemanticHead(nn.Module):
    ...
    def __init__(self,
        ...
        assert with_semantic or with_reg
        ...
        if init_cfg is None:
            ...
            self.init_cfg += [
                dict(
                    ...
                    override=[
                        dict(name='semantic_fcs'),
                        dict(name='reg_fcs')
                    ])
            ]
    @property
    def custom_semantic_channels(self):
        return getattr(self.loss_semantic, 'custom_semantic_channels', False)

    @property
    def custom_activation(self):
        return getattr(self.loss_semantic, 'custom_activation', False)

    @property
    def custom_accuracy(self):
        return getattr(self.loss_semantic, 'custom_accuracy', False)
    ...
    def loss(self,
        ...
        if semantic_score is not None:
            ...
            if semantic_score.numel() > 0:
                loss_semantic_ = self.loss_semantic(
                    semantic_score,
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override)
                if isinstance(loss_semantic_, dict):
                    losses.update(loss_semantic_)
                else:
                    losses['loss_semantic'] = loss_semantic_
                if self.custom_activation:
                    acc_ = self.loss_semantic.get_accuracy(semantic_score, labels)
                    losses.update(acc_)
                else:
    ...
    # get_bboxes in mmdet2.0, get_det_bboxes in mmdet1.0
    @force_fp32(apply_to=('semantic_score', 'bbox_pred'))
    def get_bboxes(self,
                   rois,
                   semantic_score,
        ...
        if self.custom_semantic_channels:
            scores = self.loss_semantic.get_activation(semantic_score)
        else:
            scores = F.softmax(
                semantic_score, dim=-1) if semantic_score is not None else None
    ...
    def onnx_export(self,
        ...
        if self.custom_cls_channels:
            scores = self.loss_semantic.get_activation(cls_score)
        else:
            scores = F.softmax(
