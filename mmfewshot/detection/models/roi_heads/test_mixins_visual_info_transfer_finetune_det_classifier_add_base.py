...
class BBoxTestMixin_visual_info_transfer_finetune_det_classifier_add_base: 
    ...
    def simple_test_bboxes(self,
        ...
        ft_det_cls_score = bbox_results['ft_det_cls_score']


        # 包括背景
        ft_det_cls_score = F.softmax(ft_det_cls_score, dim=1)
        # 排除背景类别的影响
        # ft_det_cls_score = F.softmax(ft_det_cls_score[:, :-1], dim=1)
        ...
        ## merge finetune det classifier and i original classifier
        ...
        COCO_ALL_CLASSES = list(range(80))
        COCO_BASE_CLASSES = list(set(COCO_ALL_CLASSES_AND_BG)-set(COCO_FS_CLASSES_AND_BG))
        
        # （1）用检测分类器排除背景类，再用生成分类器赋值前景概率
        # val_cls_all, ind_cls_all = torch.max(cls_score, 1)
        # if torch.sum(ind_cls_all!=80)> 0:
        #     val_a, ind_a = torch.max(cls_score[np.ix_((ind_cls_all!=80).cpu().numpy(), COCO_ALL_CLASSES)], 1)
        #     val_b, ind_b = torch.max(ft_det_cls_score[np.ix_((ind_cls_all!=80).cpu().numpy(), COCO_ALL_CLASSES)], 1)
        #     # print(ind_a)
        #     # print(ind_b)
        #     if torch.sum(ind_a !=  ind_b) > 0:
        #         print(torch.sum(ind_a !=  ind_b))
        #         print(ind_a[ind_a !=  ind_b], print(ind_b[ind_a !=  ind_b]))

        #     # ipdb.set_trace()
        #     cls_score[np.ix_((ind_cls_all!=80).cpu().numpy(), COCO_ALL_CLASSES)] = ft_det_cls_score[np.ix_((ind_cls_all!=80).cpu().numpy(), COCO_ALL_CLASSES)]

        # （2）用生成分类器排除背景类，再用检测分类器赋值前景概率
        val_cls_all, ind_cls_all = torch.max(cls_score, 1)
        val_ft_det, ind_ft_det = torch.max(ft_det_cls_score, 1)
        ft_det_cls_score_exclude_bg = F.softmax(ft_det_cls_score[:, :-1], dim=1)
        cls_score_exclude_bg = F.softmax(cls_score[:, :-1], dim=1)
        if torch.sum(ind_ft_det!=80)> 0:
            # val_a, ind_a = torch.max(cls_score_exclude_bg[np.ix_((ind_ft_det!=80).cpu().numpy(), COCO_ALL_CLASSES)], 1)
            # val_b, ind_b = torch.max(ft_det_cls_score_exclude_bg[np.ix_((ind_ft_det!=80).cpu().numpy(), COCO_ALL_CLASSES)], 1)
            # print(ind_a)
            # print(ind_b)
            # if torch.sum(ind_a !=  ind_b) > 0:
            #     print(torch.sum(ind_a !=  ind_b))
            ...
            # ipdb.set_trace()
            # (2.1) 将ft_det认为是前景类别的cls_score概率用ft_det概率覆盖
            # cls_score[np.ix_((ind_ft_det!=80).cpu().numpy(), COCO_ALL_CLASSES_AND_BG)] = ft_det_cls_score[np.ix_((ind_ft_det!=80).cpu().numpy(), COCO_ALL_CLASSES_AND_BG)]
            # (2.2) 将ft_det认为是前景类别的且cls_score是背景类别的cls_score概率用ft_det概率覆盖
            cls_score[np.ix_(((ind_ft_det!=80)&(ind_cls_all==80)).cpu().numpy(), COCO_ALL_CLASSES_AND_BG)] = ft_det_cls_score[np.ix_(((ind_ft_det!=80)&(ind_cls_all==80)).cpu().numpy(), COCO_ALL_CLASSES_AND_BG)]
...
class MaskTestMixin_visual_info_transfer_finetune_det_classifier_add_base:
