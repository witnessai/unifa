...
class BBoxTestMixin_visual_info_transfer_direct_probabilities: 
    ...
    def simple_test_bboxes(self,
        ...
        fs_score = bbox_results['fs_score']
       
        cls_score = F.softmax(cls_score, dim=1)
        # direct probabilities
        fs_score = F.softmax(fs_score, dim=1)
        ...
        ## 测试前景类情况下 检测基类、检测小类、生成小类分类器概率值差异
        ## 
        # val_cls_bs, _ = torch.max(cls_score[:, COCO_BASE_CLASSES], 1)
        # val_cls_fs, _ = torch.max(cls_score[:, COCO_FS_CLASSES], 1)
        # val_fs_fs, _ = torch.max(fs_score[:, :-1], 1)
        # val_cls_all, ind_cls_all = torch.max(cls_score, 1)
        # print(torch.mean(val_cls_bs[ind_cls_all!=80]).cpu(), torch.mean(val_cls_fs[ind_cls_all!=80]).cpu(), torch.mean(val_fs_fs[ind_cls_all!=80]).cpu())
        ## 不同的实现方式
        ## (1) 比较两个分类器对每个类的分类概率，保留较高的那一个
        # for idx, c in enumerate(COCO_FS_CLASSES_AND_BG[:-1]):   
        #     cls_score[:, c] = torch.max(fs_score[:, idx], cls_score[:, c])
        ## (2) 先做一个判断，看两个分类器的最大概率值谁大就保留谁的所有概率分布
        # val_cls, ind_cls = torch.max(cls_score[:, COCO_FS_CLASSES_AND_BG], 1)
        # val_fs, ind_fs = torch.max(fs_score[:, :], 1)
        # cls_score[np.ix_((val_cls<val_fs).cpu().numpy(), COCO_FS_CLASSES)] = fs_score[val_cls<val_fs, :-1]
        ## (3) 如果不是背景类再替换
        val_cls_bs, _ = torch.max(cls_score[:, COCO_BASE_CLASSES], 1)
        val_cls_all, ind_cls_all = torch.max(cls_score, 1)
        # ipdb.set_trace()
        _idx = list(range(16-1))
        # ipdb.set_trace()
        cls_score[np.ix_((ind_cls_all!=80).cpu().numpy(), COCO_FS_CLASSES)] = fs_score[np.ix_((ind_cls_all!=80).cpu().numpy(), _idx)]
        
        
        # print(cls_score[np.ix_((val_cls<val_fs).cpu().numpy(), COCO_FS_CLASSES)])
        # ipdb.set_trace()
...
class MaskTestMixin_visual_info_transfer_direct_probabilities:
